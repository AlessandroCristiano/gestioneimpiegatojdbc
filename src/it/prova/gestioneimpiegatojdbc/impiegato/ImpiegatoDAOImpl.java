...
public class ImpiegatoDAOImpl extends AbstractMySQLDAO implements ImpiegatoDAO{
	...
	public int insert(Impiegato input) throws Exception {
		...
		try (PreparedStatement ps = connection.prepareStatement(
			...
			ps.setDate(4, new java.sql.Date(input.getDataNascita().getTime()));
			ps.setDate(5, new java.sql.Date(input.getDataAssunzione().getTime()));
			ps.setLong(6, input.getCompagnie().getId());
			result = ps.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
	...
	public List<Impiegato> findAllByCompagnia(Compagnia input) throws Exception {
		...
		Impiegato result = null;
		List<Impiegato> listaElementi = new ArrayList<>();
	...
	@Override
	public int countByDataFondazioneCompagniaGreaterThan(java.util.Date dateInput) throws Exception {
		if (isNotActive())
			throw new Exception("Connessione non attiva. Impossibile effettuare operazioni DAO.");
		...
		int result = 0;
		try (PreparedStatement ps = connection.prepareStatement("select count(c.id) id from impiegato i inner join compagnia c where c.datafondazione > ?;")) {
